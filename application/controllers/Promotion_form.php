...
class Promotion_form extends CI_Controller
{
	...
	public function index()
	{
		...
		if($id_form == 1){
			$this->template->load('template', 'promotion_form/sales/promotion_header_list', $data);
		} elseif($id_form == 2) {
			$this->template->load('template', 'promotion_form/marketing/promotion_header_list', $data);
		} else {
			redirect('welcome');
		}
	}
	...
	public function read($id)
	{
		...
		$row_review = $this->Promotion_header_model->get_review($promotion_number);
		...
		$row_review_mkt = $this->Promotion_header_model->get_review_mkt($promotion_number);
		$row_review_mktp = $this->Promotion_header_model->get_review_mktp($promotion_number);
		$row_brandMgr = $this->Promotion_header_model->get_brandMgr($promotion_number);
		$row_mktMgr = $this->Promotion_header_model->get_mktMgr($promotion_number);
		$row_snrMktMgr = $this->Promotion_header_model->get_snrMktMgr($promotion_number);
		$row_salesDirector = $this->Promotion_header_model->get_salesDirector($promotion_number);
		$row_financeDirector = $this->Promotion_header_model->get_financeDirector($promotion_number);
		$row_marketingDirector = $this->Promotion_header_model->get_marketingDirector($promotion_number);
		...
		if ($row) {
			$data = array(
				...
				'max_review' => $row_review->max_review,
				'brand_name' => $row->brand_name,
				'series_name' => $row->series_name,
				'row_review_mkt' => $row_review_mkt->max_review,
				'row_brandMgr' => $row_brandMgr->max_review,
				'row_salesDirector' => $row_salesDirector->max_review,
				'row_financeDirector' => $row_financeDirector->max_review,
				'gl_account_onTopPromo' => $gl_account_onTopPromo,
				'row_marketingDirector' => $row_marketingDirector->max_review,
				'row_review_mktp' => $row_review_mktp->max_review,
				'row_mktMgr' => $row_mktMgr->max_review,
				'row_snrMktMgr' => $row_snrMktMgr->max_review,
				'type' => $type,
			);
	...
	public function create()
	{
		...
		$row_business_unit = $this->Promotion_header_model->get_business_unit($kode_departemen);
		$data = array(
			'button' => 'Create',
			...
			'promo_activity_name' => set_value('promo_activity_name'),
			...
			'business_unit' => set_value('business_unit'),
			'brand_code' => set_value('brand_code'),
			'series_code' => set_value('series_code'),
			'row_business_unit' => $row_business_unit,
			'qty_gratis' => set_value('qty_gratis'),
		);
		if($id_form == 1) {
			$this->template->load('template', 'promotion_form/sales/promotion_header_form', $data);
		} elseif($id_form == 2) {
			$this->template->load('template', 'promotion_form/marketing/promotion_header_form', $data);
		} else {
	...
	*/
	public function create_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$series_code = $this->input->post('series_code', TRUE);
			$alias_series = $this->Promotion_header_model->get_alias_series($series_code);
			...
			if ($kode_departemen == '0306') {
				$promotion_number =  $alias_dept . "/" . $alias_series . "/" . $bulan . "/" . $tahun . "/" . $kode;
			} else {
			...
			if($kode_departemen == '0306' || $kode_departemen == '0306401' || $kode_departemen == '0307' || $kode_departemen == '0308' || $kode_departemen == '0303' || $kode_departemen == '0323') {
				if($type == "2") {
					/* PRODUCT GRATIS */

					$product_name_gratis = $this->input->post('product_name_gratis', TRUE);
					$qty_gratis = str_replace(".", "", $this->input->post('qty_gratis', TRUE));

					if (array_filter($product_name_gratis)) {
						$productGratisArray = array();

						$index9 = 0;
						foreach ($product_name_gratis as $data_product_gratis) {
							$add_product = array(
								'product_code' => $data_product_gratis,
								'promotion_number' => $promotion_number,
								'qty' => $qty_gratis[$index9],
								'SecLogDate' => date('Y-m-d H:i:s'),
								'SecLogUser' => $this->session->userdata('full_name')
							);
							array_push($productGratisArray, $add_product);
							$index9++;
						}
						$this->Promotion_header_model->insert_product_gratis($productGratisArray);
					...
					if (array_filter($product_name)) {
						...
						$this->Promotion_header_model->update_product($promotion_number);
					} else {
						$baseline_sales = array('0');
						$incremental_sales = array('0');
					}
			...
			*/
			...
			if ($listingActivity != NULL && $listingAccountCode != NULL && $listingAmount != NULL) {
				...
				foreach ($listing_activity as $data_listing) {
					$add_listing['listing_activity'] = $data_listing;
					$add_listing['promotion_number'] = $promotion_number;
					if ($this->input->post('kode_departemen', TRUE) == '0306') {
						if ($this->input->post('business_unit_id', TRUE) == 0 || $this->input->post('business_unit_id', TRUE) == '') {
							$add_listing['gl_coa_segment'] = "01" . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $this->input->post('brand_code', TRUE) . "-" . $this->input->post('series_code', TRUE) . "-" . $listing_account_code[$index1];
						} else {
							$add_listing['gl_coa_segment'] = "01" . "-" . $this->input->post('business_unit_id', TRUE) . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $this->input->post('brand_code', TRUE) . "-" . $this->input->post('series_code', TRUE) . "-" . $listing_account_code[$index1];
						}
					} elseif ($this->input->post('kode_departemen', TRUE) == '0306401' || $this->input->post('kode_departemen', TRUE) == '0307' || $this->input->post('kode_departemen', TRUE) == '0308' || $this->input->post('kode_departemen', TRUE) == '0303') {
						if ($this->input->post('business_unit_id', TRUE) == 0 || $this->input->post('business_unit_id', TRUE) == '') {
							$add_listing['gl_coa_segment'] = "01" . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $listing_account_code[$index1];
						} else {
							$add_listing['gl_coa_segment'] = "01" . "-" . $this->input->post('business_unit_id', TRUE) . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $listing_account_code[$index1];
						}
					} else {
						if ($this->input->post('business_unit_id', TRUE) == 0 || $this->input->post('business_unit_id', TRUE) == '') {
							$add_listing['gl_coa_segment'] = "01" . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $this->input->post('store_code', TRUE) . $this->input->post('region_code', TRUE) . "-" . $listing_account_code[$index1];
						} else {
							$add_listing['gl_coa_segment'] = "01" . "-" . $this->input->post('business_unit_id', TRUE) . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $this->input->post('store_code', TRUE) . $this->input->post('region_code', TRUE) . "-" . $listing_account_code[$index1];
						}
					}
					$add_listing['gl_account_code'] = $listing_account_code[$index1];
					$add_listing['amount'] = $listing_amount[$index1];
					if (array_sum($incremental_sales) == 0 || array_sum($incremental_sales) == '') {
						$add_listing['incremental_sales'] = 0;
					} else {
						$add_listing['incremental_sales'] = ($listing_amount[$index1] / array_sum($incremental_sales)) * 100;
					}
					$add_listing['source_fund'] = $listing_source_of_fund[$index1];
					$add_listing['remark'] = $listing_remarks[$index1];
					$add_listing['SecLogDate'] = date('Y-m-d H:i:s');
					$add_listing['SecLogUser'] = $this->session->userdata('full_name');

					array_push($listingArray, $add_listing);
					$index1++;
				}
			...
			if ($promoActivity != NULL && $promoAccountCode != NULL && $promoAmount != NULL) {
				...
				foreach ($promo_activity as $data_promo) {
					$add_promo['promo_activity_name'] = $data_promo;
					$add_promo['promotion_number'] = $promotion_number;
					if ($this->input->post('kode_departemen', TRUE) == '0306') {
						if ($this->input->post('business_unit_id', TRUE) == 0 || $this->input->post('business_unit_id', TRUE) == '') {
							$add_promo['gl_coa_segment'] = "01" . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $this->input->post('brand_code', TRUE) . "-" . $this->input->post('series_code', TRUE) . "-" . $promo_account_code[$index2];
						} else {
							$add_promo['gl_coa_segment'] = "01" . "-" . $this->input->post('business_unit_id', TRUE) . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $this->input->post('brand_code', TRUE) . "-" . $this->input->post('series_code', TRUE) . "-" . $promo_account_code[$index2];
						}
					} elseif ($this->input->post('kode_departemen', TRUE) == '0306401' || $this->input->post('kode_departemen', TRUE) == '0307' || $this->input->post('kode_departemen', TRUE) == '0308' || $this->input->post('kode_departemen', TRUE) == '0303') {
						if ($this->input->post('business_unit_id', TRUE) == 0 || $this->input->post('business_unit_id', TRUE) == '') {
							$add_promo['gl_coa_segment'] = "01" . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $promo_account_code[$index2];
						} else {
							$add_promo['gl_coa_segment'] = "01" . "-" . $this->input->post('business_unit_id', TRUE) . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $promo_account_code[$index2];
						}
					} else {
						if ($this->input->post('business_unit_id', TRUE) == 0 || $this->input->post('business_unit_id', TRUE) == '') {
							$add_promo['gl_coa_segment'] = "01" . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $this->input->post('store_code', TRUE) . $this->input->post('region_code', TRUE) . "-" . $promo_account_code[$index2];
						} else {
							$add_promo['gl_coa_segment'] = "01" . "-" . $this->input->post('business_unit_id', TRUE) . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $this->input->post('store_code', TRUE) . $this->input->post('region_code', TRUE) . "-" . $promo_account_code[$index2];
						}
					}
					$add_promo['gl_account_code'] = $promo_account_code[$index2];
					$add_promo['amount'] = $promo_amount[$index2];
					if (array_sum($incremental_sales) == 0 || array_sum($incremental_sales) == '') {
						$add_promo['incremental_sales'] = 0;
					} else {
						$add_promo['incremental_sales'] = ($promo_amount[$index2] / array_sum($incremental_sales)) * 100;
					}
					$add_promo['source_fund'] = $promo_source_of_fund[$index2];
					$add_promo['remark'] = $promo_remarks[$index2];
					$add_promo['SecLogDate'] = date('Y-m-d H:i:s');
					$add_promo['SecLogUser'] = $this->session->userdata('full_name');

					array_push($promoArray, $add_promo);
					$index2++;
				}
			...
			if ($tradingActivity != NULL && $tradingAccountCode != NULL && $tradingAmount != NULL) {
				...
				foreach ($trading_activity as $data_trading) {
					$add_trading['trading_activity_name'] = $data_trading;
					$add_trading['promotion_number'] = $promotion_number;
					if ($this->input->post('kode_departemen', TRUE) == '0306') {
						if ($this->input->post('business_unit_id', TRUE) == 0 || $this->input->post('business_unit_id', TRUE) == '') {
							$add_trading['gl_coa_segment'] = "01" . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $this->input->post('brand_code', TRUE) . "-" . $this->input->post('series_code', TRUE) . "-" . $trading_account_code[$index5];
						} else {
							$add_trading['gl_coa_segment'] = "01" . "-" . $this->input->post('business_unit_id', TRUE) . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $this->input->post('brand_code', TRUE) . "-" . $this->input->post('series_code', TRUE) . "-" . $trading_account_code[$index5];
						}
					} elseif ($this->input->post('kode_departemen', TRUE) == '0306401' || $this->input->post('kode_departemen', TRUE) == '0307' || $this->input->post('kode_departemen', TRUE) == '0308' || $this->input->post('kode_departemen', TRUE) == '0303') {
						if ($this->input->post('business_unit_id', TRUE) == 0 || $this->input->post('business_unit_id', TRUE) == '') {
							$add_trading['gl_coa_segment'] = "01" . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $trading_account_code[$index5];
						} else {
							$add_trading['gl_coa_segment'] = "01" . "-" . $this->input->post('business_unit_id', TRUE) . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $trading_account_code[$index5];
						}
					} else {
						if ($this->input->post('business_unit_id', TRUE) == 0 || $this->input->post('business_unit_id', TRUE) == '') {
							$add_trading['gl_coa_segment'] = "01" . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $this->input->post('store_code', TRUE) . $this->input->post('region_code', TRUE) . "-" . $trading_account_code[$index5];
						} else {
							$add_trading['gl_coa_segment'] = "01" . "-" . $this->input->post('business_unit_id', TRUE) . "-" . $this->input->post('kode_departemen', TRUE) . "-" . $this->input->post('channel_code', TRUE) . "-" . $this->input->post('store_code', TRUE) . $this->input->post('region_code', TRUE) . "-" . $trading_account_code[$index5];
						}
					}
					$add_trading['gl_account_code'] = $trading_account_code[$index5];
					$add_trading['amount'] = $trading_amount[$index5];
					if (array_sum($incremental_sales) == 0 || array_sum($incremental_sales) == '') {
						$add_trading['incremental_sales'] = 0;
					} else {
						$add_trading['incremental_sales'] = ($trading_amount[$index5] / array_sum($incremental_sales)) * 100;
					}
					$add_trading['SecLogDate'] = date('Y-m-d H:i:s');
					$add_trading['SecLogUser'] = $this->session->userdata('full_name');

					array_push($tradingArray, $add_trading);
					$index5++;
				}
			...
			*/
			...
			$this->insertApprovalProgram($promotion_number, $kode_departemen, $channel_code, $store_code, $region_code, $total_listing_cost, $total_on_top_promo, $business_unit_id, $brand_code, $series_code);
	...
	public function insertApprovalProgram($promotion_number, $kode_departemen, $channel_code, $store_code, $region_code, $total_listing_cost, $total_on_top_promo, $business_unit_id, $brand_code, $series_code)
	{
		/* APPROVAL PROGRAM */
		...
		if ($business_unit_id == '' || $business_unit_id == NULL) {
			if ($kode_departemen == '0312') {
				$row_approval = $this->Promotion_header_model->get_approval_ka_old($kode_departemen, $channel_code, $store_code, $total_on_top_promo, $total_budget);
			} elseif ($kode_departemen == '0305') {
				$row_approval = $this->Promotion_header_model->get_approval_tm_old($kode_departemen, $channel_code, $region_code, $store_code, $total_budget);
			} else {
				$row_approval = $this->Promotion_header_model->get_approval_ec_old($kode_departemen, $channel_code, $total_budget);
			}
		} else {
			if ($kode_departemen == '0312') {
				$row_approval = $this->Promotion_header_model->get_approval_ka($kode_departemen, $channel_code, $store_code, $total_on_top_promo, $total_budget, $business_unit_id);
			} elseif ($kode_departemen == '0305') {
				$row_approval = $this->Promotion_header_model->get_approval_tm($kode_departemen, $channel_code, $region_code, $store_code, $total_budget, $business_unit_id);
			} elseif ($kode_departemen == '0323') {
				$row_approval = $this->Promotion_header_model->get_approval_ec($kode_departemen, $channel_code, $store_code, $total_on_top_promo, $total_budget, $business_unit_id);
			} elseif ($kode_departemen == '0306') {
				$row_approval = $this->Promotion_header_model->get_approval_mkt($kode_departemen, $brand_code, $series_code, $total_budget, $business_unit_id);
			} elseif ($kode_departemen == '0303') {
				$row_approval = $this->Promotion_header_model->get_approval_pr($kode_departemen, $total_budget, $business_unit_id);
			} else {
				$row_approval = $this->Promotion_header_model->get_approval_mktp($kode_departemen, $total_budget, $business_unit_id);
			}
		}
	...
	*/
	public function update($id)
	{
		...
		$row_onTopPromo = $this->Promotion_header_model->get_on_top_promo($row->promotion_number, $kode_departemen);
		...
		if ($row->business_unit_id == '' || $row->business_unit_id == 0 || $row->business_unit_id == NULL) {
			if ($row->kode_departemen == '0306') {
				$gl_coa_segment = "01" . "-" . $row->kode_departemen . "-" . $row->channel_code . "-" . $row->brand_code . "-" . $row->series_code;
			} elseif ($row->kode_departemen == '0306401' || $row->kode_departemen == '0308' || $row->kode_departemen == '0307' || $row->kode_departemen == '0303') {
				$gl_coa_segment = "01" . "-" . $row->kode_departemen . "-" . $row->channel_code;
			} else {
				if ($row->channel_code == '001' || $row->channel_code == '004') {
					$gl_coa_segment = "01" . "-" . $row->kode_departemen . "-" . $row->channel_code . "-" . $row->store_code;
				} else if ($row->channel_code == '002' || $row->channel_code == '003') {
					$gl_coa_segment = "01" . "-" . $row->kode_departemen . "-" . $row->channel_code . "-" . $row->region_code;
				}
			}
		} else {
			if ($row->kode_departemen == '0306') {
				$gl_coa_segment = "01" . "-" . $row->business_unit_id . "-" . $row->kode_departemen . "-" . $row->channel_code . "-" . $row->brand_code . "-" . $row->series_code;
			} elseif ($row->kode_departemen == '0306401' || $row->kode_departemen == '0308' || $row->kode_departemen == '0307' || $row->kode_departemen == '0303') {
				$gl_coa_segment = "01" . "-" . $row->business_unit_id . "-" . $row->kode_departemen . "-" . $row->channel_code;
			} else {
				if ($row->channel_code == '001' || $row->channel_code == '004') {
					$gl_coa_segment = "01" . "-" . $row->business_unit_id . "-" . $row->kode_departemen . "-" . $row->channel_code . "-" . $row->store_code;
				} else if ($row->channel_code == '002' || $row->channel_code == '003') {
					$gl_coa_segment = "01" . "-" . $row->business_unit_id . "-" . $row->kode_departemen . "-" . $row->channel_code . "-" . $row->region_code;
				}
			}
		}
		...
		if ($row) {
			$data = array(
				...
				'row_departemen' => $row_departemen,
				'business_unit' => set_value('business_unit', $row->business_unit_id),
				'row_product' => $row_product,
				...
				'business_unit_name' => set_value('business_unit_name', $row->business_unit_name),
	...
	public function update_action()
	{
		...
		$kode_distributor = $this->input->post('kode_distributor', TRUE);
		if ($row_promotion->business_unit_id == '' || $row_promotion->business_unit_id == 0 || $row_promotion->business_unit_id == NULL) {
			if ($row_promotion->kode_departemen == '0306') {
				$gl_coa_segment = "01" . "-" . $row_promotion->kode_departemen . "-" . $row_promotion->channel_code . "-" . $row_promotion->brand_code . "-" . $row_promotion->series_code;
			} elseif ($row_promotion->kode_departemen == '0306401' || $row_promotion->kode_departemen == '0308' || $row_promotion->kode_departemen == '0307' || $row_promotion->kode_departemen == '0303') {
				$gl_coa_segment = "01" . "-" . $row_promotion->kode_departemen . "-" . $row_promotion->channel_code;
			} else {
				...
				} else if ($row_promotion->channel_code == '002' || $row_promotion->channel_code == '003') {
					$gl_coa_segment = "01" . "-" . $row_promotion->kode_departemen . "-" . $row_promotion->channel_code . "-" . $row_promotion->region_code;
				}
			}
		} else {
			if ($row_promotion->kode_departemen == '0306') {
				$gl_coa_segment = "01" . "-" . $row_promotion->business_unit_id . "-" . $row_promotion->kode_departemen . "-" . $row_promotion->channel_code . "-" . $row_promotion->brand_code . "-" . $row_promotion->series_code;
			} elseif ($row_promotion->kode_departemen == '0306401' || $row_promotion->kode_departemen == '0308' || 	$row_promotion->kode_departemen == '0307' || $row_promotion->kode_departemen == '0303') {
				$gl_coa_segment = "01" . "-" . $row_promotion->business_unit_id . "-" . $row_promotion->kode_departemen . "-" . $row_promotion->channel_code;
			} else {
				if ($row_promotion->channel_code == '001' || $row_promotion->channel_code == '004') {
					$gl_coa_segment = "01" . "-" . $row_promotion->business_unit_id . "-" . $row_promotion->kode_departemen . "-" . $row_promotion->channel_code . "-" . $row_promotion->store_code;
				} else if ($row_promotion->channel_code == '002' || $row_promotion->channel_code == '003') {
					$gl_coa_segment = "01" . "-" . $row_promotion->business_unit_id . "-" . $row_promotion->kode_departemen . "-" . $row_promotion->channel_code . "-" . $row_promotion->region_code;
				}
			}
		}
		$config['upload_path'] = './uploads/';
		...
		*/
		...
		if ($listingActivity != NULL && $listingAccountCode != NULL && $listingAmount != NULL) {
			...
			foreach ($listing_activity as $data_listing) {
				...
				if (array_sum($incremental_sales) == 0 || array_sum($incremental_sales) == '') {
					$add_data['incremental_sales'] = 0;
				} else {
					$add_data['incremental_sales'] = ($listing_amount[$index1] / array_sum($incremental_sales)) * 100;
				}
		...
		if ($promoActivity != NULL && $promoAccountCode != NULL && $promoAmount != NULL) {
			...
			foreach ($promo_activity as $data_promo) {
				...
				if (array_sum($incremental_sales) == 0 || array_sum($incremental_sales) == '') {
					...
					$add_data['incremental_sales'] = ($promo_amount[$index2] / array_sum($incremental_sales)) * 100;
				}
	...
	function get_store()
	{
		...
		$kode_departemen = $this->input->post('kode_departemen', TRUE);
		$business_unit_id = $this->input->post('business_unit_id', TRUE);
		$data = $this->Promotion_header_model->get_store($channel_code, $kode_departemen, $business_unit_id)->result();
		echo json_encode($data);
	}
	...
	function get_region()
	{
		...
		$kode_departemen = $this->input->post('kode_departemen', TRUE);
		$business_unit_id = $this->input->post('business_unit_id', TRUE);
		$data = $this->Promotion_header_model->get_region($channel_code, $kode_departemen, $business_unit_id)->result();
		echo json_encode($data);
	}
	...
	{
		$business_unit_id = $this->input->post('id', TRUE);
		$data = $this->Promotion_header_model->get_product_category_1($business_unit_id)->result();
		echo json_encode($data);
	}
	...
	{
		$business_unit_id = $this->input->post('id', TRUE);
		$data = $this->Promotion_header_model->get_product_category_2($business_unit_id)->result();
		echo json_encode($data);
	}
	...
	{
		$business_unit_id = $this->input->post('id', TRUE);
		$data = $this->Promotion_header_model->get_product_name($business_unit_id)->result();
		echo json_encode($data);
	}
	...
	{
		$business_unit_id = $this->input->post('id', TRUE);
		$data = $this->Promotion_header_model->get_business_id($business_unit_id)->result();
		echo json_encode($data);
	}
	...
	{
		$kode_departemen = $this->input->post('id', TRUE);
		$data = $this->Promotion_header_model->get_brand($kode_departemen)->result();
		echo json_encode($data);
	}
	...
	function get_series()
	{
		...
		$kode_departemen = $this->input->post('kode_departemen', TRUE);
		$data = $this->Promotion_header_model->get_series($brand_code, $kode_departemen)->result();
		echo json_encode($data);
	}
	...
	public function export_excel()
	{
		...
		$sheet->setCellValue('E1', "Promotion Number");
		$sheet->setCellValue('F1', "Promotion Name");
		$sheet->setCellValue('G1', "Start Period");
		$sheet->setCellValue('H1', "End Period");
		$sheet->setCellValue('I1', "Fiscal Year");
		$sheet->setCellValue('J1', "Channel Name");
		$sheet->setCellValue('K1', "Region Name");
		$sheet->setCellValue('L1', "Area Name");
		$sheet->setCellValue('M1', "Store Name");
		$sheet->setCellValue('N1', "Coa");
		$sheet->setCellValue('O1', "Trading Term");
		$sheet->setCellValue('P1', "Listing Cost");
		$sheet->setCellValue('Q1', "On Top Promo");
		$sheet->setCellValue('R1', "Amount All Cost");
		$sheet->setCellValue('S1', "Product Baseline");
		$sheet->setCellValue('T1', "Product Incremental");
		$sheet->setCellValue('U1', "Amount Sales");
		...
		foreach ($promotion as $data) {
			...
			$sheet->setCellValue('E' . $numrow, $data->promotion_number);
			$sheet->setCellValue('F' . $numrow, $data->promotion_name);
			$sheet->setCellValue('G' . $numrow, $data->periode_awal);
			$sheet->setCellValue('H' . $numrow, $data->periode_akhir);
			$sheet->setCellValue('I' . $numrow, $data->fiscal_year);
			$sheet->setCellValue('J' . $numrow, $data->channel_name);
			$sheet->setCellValue('K' . $numrow, $data->nama_region);
			$sheet->setCellValue('L' . $numrow, $data->nama_area);
			$sheet->setCellValue('M' . $numrow, $data->store_name);
			$sheet->setCellValue('N' . $numrow, $data->gl_account_code);
			$sheet->setCellValue('O' . $numrow, $data->amount_trading);
			$sheet->setCellValue('P' . $numrow, $data->amount_listing);
			$sheet->setCellValue('Q' . $numrow, $data->amount_promo);
			$sheet->setCellValue('R' . $numrow, $data->all_cost);
			$sheet->setCellValue('S' . $numrow, $data->total_product_baseline);
			$sheet->setCellValue('T' . $numrow, $data->total_product_incremental);
			$sheet->setCellValue('U' . $numrow, $data->amount_sales);
			$sheet->setCellValue('V' . $numrow, $data->status_name);
		...
		$sheet2->setCellValue('F1', "Business Unit");
		$sheet2->setCellValue('G1', "Category 1");
		$sheet2->setCellValue('H1', "Category 2");
		$sheet2->setCellValue('I1', "Product Code");
		$sheet2->setCellValue('J1', "Product Name");
		$sheet2->setCellValue('K1', "Baseline Sales");
		$sheet2->setCellValue('L1', "Incremental Sales");
		...
		$sheet2->getStyle('L1')->applyFromArray($styleArray);
		...
		foreach ($product as $dataProduct) {
			...
			$sheet2->setCellValue('F' . $numrow, $dataProduct->business_unit_name);
			$sheet2->setCellValue('G' . $numrow, $dataProduct->category_name1);
			$sheet2->setCellValue('H' . $numrow, $dataProduct->category_name2);
			$sheet2->setCellValue('I' . $numrow, $dataProduct->product_code);
			$sheet2->setCellValue('J' . $numrow, $dataProduct->product_name);
			$sheet2->setCellValue('K' . $numrow, $dataProduct->baseline_sales);
			$sheet2->setCellValue('L' . $numrow, $dataProduct->incremental_sales);
			...
			$sheet2->getStyle('L' . $numrow)->applyFromArray($styleArray);
